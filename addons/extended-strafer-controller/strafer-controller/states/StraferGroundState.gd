...
func process_state(delta : float, controller : BaseController3D) -> void:
    ...
    if controller.velocity.length() > SLIDE_SPEED \
        and Input.is_action_pressed("crouch") \
        and slide_state != null \
        and controller.slide_cooldown_timer <= 0.0:		# Transition to the slide start, start a slide if it hasn't been started
...
static func to_slide(controller : BaseController3D, start_slide : bool = false) -> bool:
    # HARD BLOCK: do not allow slide if cooldown active
    if controller.slide_cooldown_timer > 0.0:
        return false
